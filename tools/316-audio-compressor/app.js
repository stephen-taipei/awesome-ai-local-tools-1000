...
function setupSliders() {
    ...
    document.getElementById('makeupGain').addEventListener('input', (e) => { document.getElementById('gainValue').textContent = '+' + e.target.value + ' dB'; });
}
...
function switchLang(lang) {
    ...
    labels[4].textContent = t.makeupGain;
...
async function processAudio() {
    ...
    const makeupGainDB = parseFloat(document.getElementById('makeupGain').value);
    ...
    const makeupGain = dBToLinear(makeupGainDB);
    ...
    for (let ch = 0; ch < channels; ch++) {
        ...
        for (let i = 0; i < length; i++) {
            ...
            outputData[i] = inputData[i] * gain * makeupGain;
        }
    }
...
function downloadAudio() {
    ...
    a.download = 'compressed-audio.wav';
    a.click();
}
