/**
 * Audio Vibrato - Tool #307
 * Add vibrato (pitch modulation) effect to audio
 */
...
const texts = {
    ...
    en: {
        title: 'Vibrato',
        subtitle: 'Add pitch modulation effect to audio',
...
async function processAudio() {
    ...
    // Vibrato uses a delay line with modulated delay time
    const maxDelay = 0.02; // 20ms max delay
    const delayBufferSize = Math.ceil(maxDelay * sampleRate);
    ...
    for (let ch = 0; ch < channels; ch++) {
        ...
        for (let i = 0; i < length; i++) {
            ...
            const delayTime = maxDelay * depth * (0.5 + 0.5 * lfo);
            const delaySamples = delayTime * sampleRate;

            // Write to delay buffer
            delayBuffer[writeIndex] = inputData[i];
            ...
            const sample0 = delayBuffer[index0 % delayBufferSize];
            const sample1 = delayBuffer[index1];
            const wetSample = sample0 + (sample1 - sample0) * frac;

            outputData[i] = wetSample * mix + inputData[i] * (1 - mix);
