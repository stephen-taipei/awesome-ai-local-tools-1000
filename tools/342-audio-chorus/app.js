...
const texts = {
    ...
    en: {
        title: 'Audio Chorus',
        subtitle: 'Add chorus effect for richer sound',
...
function switchLang(lang) {
    ...
    document.getElementById('voicesLabel').textContent = t.voices;
...
async function processAudio() {
    ...
    const depthMs = parseInt(document.getElementById('depth').value);
    ...
    const baseDelayMs = 20; // Base delay in ms
    ...
    for (let ch = 0; ch < channels; ch++) {
        ...
        for (let i = 0; i < length; i++) {
            ...
            delayBuffer[i % maxDelay] = inputData[i];

            let wetSignal = 0;

            // Process each voice
            for (let v = 0; v < numVoices; v++) {
                // Calculate LFO for this voice (with phase offset)
                const phaseOffset = (v / numVoices) * 2 * Math.PI;
                const lfo = Math.sin(2 * Math.PI * rate * (i / sampleRate) + phaseOffset);

                // Calculate delay time for this voice
                const voiceDelay = baseDelaySamples + (lfo + 1) * 0.5 * depthSamples;
                const delaySample = i - voiceDelay;

                if (delaySample >= 0) {
                    // Linear interpolation for fractional delay
                    const delayIndex = Math.floor(delaySample) % maxDelay;
                    const nextIndex = (delayIndex + 1) % maxDelay;
                    const frac = delaySample - Math.floor(delaySample);

                    const sample = delayBuffer[delayIndex] * (1 - frac) + delayBuffer[nextIndex] * frac;
                    wetSignal += sample / numVoices;
                }
            }

            // Mix dry and wet signals
            outputData[i] = inputData[i] * (1 - mix) + wetSignal * mix;
        }
    }
