...
function switchLang(lang) {
    ...
    labels[2].textContent = t.feedback;
...
async function processAudio() {
    ...
    const maxDelay = 0.007; // 7ms max delay for flanger
    const minDelay = 0.001; // 1ms min delay
    const delayBufferSize = Math.ceil(maxDelay * sampleRate * 2);
    ...
    for (let ch = 0; ch < channels; ch++) {
        ...
        for (let i = 0; i < length; i++) {
            ...
            const delayTime = minDelay + (maxDelay - minDelay) * depth * (0.5 + 0.5 * lfo);
            const delaySamples = delayTime * sampleRate;

            // Write to delay buffer with feedback
            delayBuffer[writeIndex] = inputData[i] + feedbackSample * feedback;
            ...
            const delaySample = sample0 + (sample1 - sample0) * frac;

            feedbackSample = delaySample;

            // Mix dry and wet
            outputData[i] = inputData[i] * (1 - mix) + delaySample * mix;
