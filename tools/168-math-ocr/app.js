...
function formatMathExpression(text) {
    // Convert common OCR mistakes to math symbols
    let formatted = text
        .replace(/x/gi, '×')
        .replace(/\*/g, '×')
        .replace(/\//g, '÷')
...
async function performOCR() {
    ...
    try {
        ...
        const formatted = formatMathExpression(text);

        document.getElementById('resultSection').style.display = 'block';
        document.getElementById('mathDisplay').textContent = formatted || '(未偵測到公式)';
        document.getElementById('resultText').textContent = text || '(原始文字)';
