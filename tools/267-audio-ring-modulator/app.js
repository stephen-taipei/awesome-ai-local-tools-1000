...
const texts = {
    ...
    en: {
        title: 'Audio Ring Modulator',
        subtitle: 'Add ring modulation effect to audio',
...
function init() {
    ...
    document.getElementById('downloadBtn').addEventListener('click', downloadRingMod);
}
...
function switchLang(lang) {
    ...
    document.querySelectorAll('.option-group label')[1].textContent = t.waveform;
...
function getCarrierValue(phase, waveform) {
...
function applyRingMod(buffer) {
    ...
    for (let ch = 0; ch < buffer.numberOfChannels; ch++) {
        ...
        for (let i = 0; i < buffer.length; i++) {
            ...
            const carrier = getCarrierValue(phase, waveform);
            const wet = input[i] * carrier;
...
async function startPreview() {
    ...
    const ringModBuffer = applyRingMod(originalBuffer);

    sourceNode = audioContext.createBufferSource();
    sourceNode.buffer = ringModBuffer;
...
async function downloadRingMod() {
    if (!originalBuffer) return;

    const ringModBuffer = applyRingMod(originalBuffer);
    const wavBlob = bufferToWav(ringModBuffer);

    const url = URL.createObjectURL(wavBlob);
    const a = document.createElement('a');
    a.href = url;
    a.download = `${fileName}-ringmod.wav`;
    a.click();
