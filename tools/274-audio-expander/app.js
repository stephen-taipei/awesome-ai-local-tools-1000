/**
 * Audio Expander - Tool #274
 * Expand dynamic range of audio
 */
...
const texts = {
    ...
    en: {
        ...
        subtitle: 'Expand dynamic range of audio',
...
function init() {
    ...
    document.getElementById('ratioSlider').addEventListener('input', (e) => {
        document.getElementById('ratioValue').textContent = '1:' + e.target.value;
    });
    ...
    document.getElementById('downloadBtn').addEventListener('click', downloadExpanded);
}
...
function switchLang(lang) {
    ...
    document.querySelectorAll('.option-group label')[1].textContent = t.ratio;
    document.querySelectorAll('.option-group label')[2].textContent = t.attack;
    document.querySelectorAll('.option-group label')[3].textContent = t.release;
    ...
    for (let ch = 0; ch < buffer.numberOfChannels; ch++) {
        ...
        for (let i = 0; i < buffer.length; i++) {
            ...
            if (absInput > envelope) {
                envelope = envelope * attackCoeff + absInput * (1 - attackCoeff);
            } else {
                envelope = envelope * releaseCoeff + absInput * (1 - releaseCoeff);
            }

            // Calculate expansion gain
            // Below threshold: reduce gain proportionally
            let targetGain = 1;
            if (envelope < threshold && envelope > 0.0001) {
                const underDb = 20 * Math.log10(envelope / threshold);
                const reductionDb = underDb * (ratio - 1);
...
async function startPreview() {
    ...
    const expandedBuffer = applyExpander(originalBuffer);

    sourceNode = audioContext.createBufferSource();
    sourceNode.buffer = expandedBuffer;
...
async function downloadExpanded() {
    ...
    const expandedBuffer = applyExpander(originalBuffer);
    const wavBlob = bufferToWav(expandedBuffer);

    const url = URL.createObjectURL(wavBlob);
    const a = document.createElement('a');
    a.href = url;
    a.download = `${fileName}-expanded.wav`;
    a.click();
