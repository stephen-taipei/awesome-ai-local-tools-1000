...
const texts = {
    ...
    en: {
        title: 'Pitch Shifter',
        subtitle: 'Shift audio pitch up or down',
...
function setupControls() {
    document.getElementById('semitones').addEventListener('input', (e) => {
        ...
        document.getElementById('semitonesValue').textContent = (val > 0 ? '+' : '') + val;
    });
    document.getElementById('cents').addEventListener('input', (e) => {
        const val = parseInt(e.target.value);
        document.getElementById('centsValue').textContent = (val > 0 ? '+' : '') + val + ' cents';
    });
}
...
function switchLang(lang) {
    ...
    labels[0].textContent = t.semitones;
    labels[1].textContent = t.cents;
...
async function processAudio() {
    ...
    const cents = parseInt(document.getElementById('cents').value);
    ...
    // Resample to change pitch while keeping duration
    // This simple method changes both pitch and speed, then we compensate
    const newSampleRate = Math.round(originalSampleRate * pitchRatio);
    const newLength = Math.round(originalLength / pitchRatio);

    processedBuffer = audioContext.createBuffer(channels, newLength, originalSampleRate);
...
function downloadAudio() {
    ...
    a.download = 'pitch-shifted-audio.wav';
    a.click();
}
