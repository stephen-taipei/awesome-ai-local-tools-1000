/**
 * Audio Low-Pass Filter - Tool #323
 * Remove high frequency noise and harshness
 */
...
const texts = {
    ...
    en: {
        ...
        subtitle: 'Remove high frequency noise and harshness',
...
function setupSliders() {
    document.getElementById('cutoff').addEventListener('input', (e) => {
        ...
        document.getElementById('cutoffValue').textContent = val >= 1000 ? (val/1000).toFixed(1) + ' kHz' : val + ' Hz';
    });
    document.getElementById('q').addEventListener('input', (e) => {
        document.getElementById('qValue').textContent = parseFloat(e.target.value).toFixed(2);
    });
}
...
function switchLang(lang) {
    ...
    labels[0].textContent = t.cutoff;
    labels[1].textContent = t.q;
    labels[2].textContent = t.slope;
...
// Low-pass filter coefficients (biquad)
function calculateLowPassCoeffs(sampleRate, cutoff, Q) {
...
async function processAudio() {
    ...
    // Number of filter passes (12dB = 1 pass, 24dB = 2 passes, etc.)
    const passes = slope / 12;
    const coeffs = calculateLowPassCoeffs(sampleRate, cutoff, Q);
    ...
    for (let ch = 0; ch < channels; ch++) {
        ...
        // Apply filter multiple times for steeper slopes
