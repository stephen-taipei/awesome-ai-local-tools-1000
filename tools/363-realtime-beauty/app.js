/**
 * Realtime Beauty Filter - Tool #363
 * Apply beauty filters to webcam in real-time
 */
...
let recordedChunks = [];
let settings = { smooth: 50, brighten: 30, slim: 0, eye: 0 };
...
const texts = {
    ...
    en: {
        title: 'Realtime Beauty Filter',
        subtitle: 'Real-time beauty filter effects',
...
function init() {
    ...
    ['smooth', 'brighten', 'slim', 'eye'].forEach(key => {
...
async function toggleCamera() {
    ...
    try {
        ...
        document.getElementById('beautyControls').style.display = 'block';
...
function onResults(results) {
    ...
    if (settings.smooth > 0 || settings.brighten > 0) {
        const imageData = ctx.getImageData(0, 0, canvas.width, canvas.height);
        const data = imageData.data;

        // Apply skin smoothing (simple blur approximation)
        if (settings.smooth > 0) {
            const smoothFactor = settings.smooth / 100;
            // Simple brightness/contrast adjustment for skin smoothing effect
            for (let i = 0; i < data.length; i += 4) {
                ...
                if (r > 95 && g > 40 && b > 20 && r > g && r > b && Math.abs(r - g) > 15) {
                    // Smooth skin by reducing contrast
                    const avg = (r + g + b) / 3;
        ...
        if (settings.brighten > 0) {
            const brightenFactor = settings.brighten / 100 * 30;
            for (let i = 0; i < data.length; i += 4) {
    ...
    if (results.multiFaceLandmarks && results.multiFaceLandmarks.length > 0) {
        // Face mesh detected - could add more advanced effects here
    }
}
...
function stopCamera() {
    ...
    document.getElementById('beautyControls').style.display = 'none';
}
