/**
 * Weapon Detection - Tool #454
 * Detect weapons in images for security purposes
 */
...
const weaponTypes = ['knife', 'gun', 'rifle', 'sword'];

const texts = {
    zh: {
        title: '武器偵測',
        subtitle: '偵測圖片中的武器（安全用途）',
...
async function detect() {
    ...
    if (hasDanger) {
        ...
        for (let i = 0; i < numDetections; i++) {
            ...
            ctx.fillRect(x, y - 22, 50, 22);
            ctx.fillStyle = '#fff';
            ctx.font = '11px sans-serif';
            ctx.fillText(t[type], x + 5, y - 7);
        }
    }
...
function displayResults(hasDanger) {
    ...
    if (hasDanger) {
        ...
        document.getElementById('detectionList').innerHTML = detectionResults.map(r => `
...
function downloadResult() {
    ...
    a.download = `weapon-detection-${Date.now()}.png`;
    a.click();
}
...
function exportReport() {
    ...
    a.download = `weapon-report-${Date.now()}.json`;
    a.click();
}
