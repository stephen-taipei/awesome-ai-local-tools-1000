/**
 * Audio Band-Pass Filter - Tool #324
...
const texts = {
    ...
    en: {
        ...
        subtitle: 'Keep only specific frequency range',
...
function setupSliders() {
    document.getElementById('center').addEventListener('input', (e) => {
        ...
        document.getElementById('centerValue').textContent = val >= 1000 ? (val/1000).toFixed(1) + ' kHz' : val + ' Hz';
    });
    document.getElementById('bandwidth').addEventListener('input', (e) => {
        document.getElementById('bandwidthValue').textContent = e.target.value + ' oct';
    });
...
function switchLang(lang) {
    ...
    labels[0].textContent = t.center;
    labels[1].textContent = t.bandwidth;
    labels[2].textContent = t.q;
...
// Band-pass filter coefficients (biquad)
function calculateBandPassCoeffs(sampleRate, centerFreq, Q) {
...
async function processAudio() {
    ...
    const centerFreq = parseFloat(document.getElementById('center').value);
    ...
    // Calculate low and high cutoff from center and bandwidth
    const lowFreq = centerFreq / Math.pow(2, bandwidth / 2);
    const highFreq = centerFreq * Math.pow(2, bandwidth / 2);

    // Use single bandpass filter
    const coeffs = calculateBandPassCoeffs(sampleRate, centerFreq, Q);
