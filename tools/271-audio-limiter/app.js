/**
 * Audio Limiter - Tool #271
 * Apply limiter effect to audio
 */
...
const texts = {
    ...
    en: {
        title: 'Audio Limiter',
        subtitle: 'Limit audio peak levels',
...
function init() {
    ...
    document.getElementById('downloadBtn').addEventListener('click', downloadLimited);
}
...
function switchLang(lang) {
    ...
    document.querySelectorAll('.option-group label')[1].textContent = t.release;
    document.querySelectorAll('.option-group label')[2].textContent = t.gain;
...
function applyLimiter(buffer) {
    ...
    const makeupGain = Math.pow(10, gainDb / 20);
    ...
    for (let ch = 0; ch < buffer.numberOfChannels; ch++) {
        ...
        for (let i = 0; i < buffer.length; i++) {
            const inputSample = input[i] * makeupGain;
            const absInput = Math.abs(inputSample);

            // Envelope follower
            if (absInput > envelope) {
                envelope = absInput; // Instant attack
            } else {
                envelope = envelope * releaseCoeff; // Smooth release
            }
...
async function startPreview() {
    ...
    const limitedBuffer = applyLimiter(originalBuffer);

    sourceNode = audioContext.createBufferSource();
    sourceNode.buffer = limitedBuffer;
...
async function downloadLimited() {
    if (!originalBuffer) return;

    const limitedBuffer = applyLimiter(originalBuffer);
    const wavBlob = bufferToWav(limitedBuffer);

    const url = URL.createObjectURL(wavBlob);
    const a = document.createElement('a');
    a.href = url;
    a.download = `${fileName}-limited.wav`;
    a.click();
