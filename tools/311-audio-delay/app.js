...
const texts = {
    ...
    en: {
        title: 'Delay',
        subtitle: 'Add delay effect to audio',
...
function setupSliders() {
    document.getElementById('delayTime').addEventListener('input', (e) => { document.getElementById('delayValue').textContent = e.target.value + ' ms'; });
    document.getElementById('feedback').addEventListener('input', (e) => { document.getElementById('feedbackValue').textContent = e.target.value + '%'; });
...
function switchLang(lang) {
    ...
    labels[0].textContent = t.delayTime;
    labels[1].textContent = t.feedback;
    labels[2].textContent = t.mix;
...
async function processAudio() {
    ...
    const extraLength = Math.ceil(delaySamples * (1 / (1 - feedback + 0.01)));
    const outputLength = originalBuffer.length + Math.min(extraLength, sampleRate * 3);
    ...
    for (let ch = 0; ch < channels; ch++) {
        ...
        for (let i = 0; i < outputLength; i++) {
            ...
            const delaySample = delayBuffer[writeIndex];
            delayBuffer[writeIndex] = drySample + delaySample * feedback;
            writeIndex = (writeIndex + 1) % delaySamples;
            outputData[i] = drySample * (1 - mix) + delaySample * mix;
        }
    }
