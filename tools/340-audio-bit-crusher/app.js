/**
 * Audio Bit Crusher - Tool #340
 * Create lo-fi effects by reducing bit depth and sample rate
 */
...
const texts = {
    ...
    en: {
        title: 'Bit Crusher',
        subtitle: 'Create retro Lo-Fi effects',
...
function setupControls() {
    document.getElementById('bitDepth').addEventListener('input', (e) => {
        document.getElementById('bitDepthValue').textContent = e.target.value + ' bit';
    });
...
function switchLang(lang) {
    ...
    document.getElementById('bitDepthLabel').textContent = t.bitDepth;
    document.getElementById('downsampleLabel').textContent = t.downsample;
...
async function processAudio() {
    ...
    // Calculate bit reduction factor
    ...
    for (let ch = 0; ch < channels; ch++) {
        ...
        for (let i = 0; i < length; i++) {
            ...
            // Downsample: hold sample for 'downsample' samples
            if (i % downsample === 0) {
                // Bit crush: reduce bit depth
                // Map from [-1, 1] to [0, levels-1], round, then map back
                sample = Math.round((sample + 1) / 2 * (levels - 1)) / (levels - 1) * 2 - 1;
                lastSample = sample;
            } else {
                sample = lastSample;
            }
