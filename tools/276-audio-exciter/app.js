...
const texts = {
    ...
    en: {
        title: 'Audio Exciter',
        subtitle: 'Add harmonic excitement to audio',
...
function init() {
    ...
    document.getElementById('downloadBtn').addEventListener('click', downloadExcited);
}
...
function switchLang(lang) {
    ...
    document.querySelectorAll('.option-group label')[1].textContent = t.harmonics;
...
function applyExciter(buffer) {
    ...
    for (let ch = 0; ch < buffer.numberOfChannels; ch++) {
        ...
        for (let i = 0; i < buffer.length; i++) {
            ...
            const saturated = Math.tanh(filtered * (1 + harmonicsAmount * 3)) * harmonicsAmount;

            // Add 2nd and 3rd harmonics
            const harmonics = saturated + Math.abs(saturated) * 0.3;

            // Mix with original
            output[i] = input[i] + harmonics * mix;
        }
...
async function startPreview() {
    ...
    const excitedBuffer = applyExciter(originalBuffer);

    sourceNode = audioContext.createBufferSource();
    sourceNode.buffer = excitedBuffer;
...
async function downloadExcited() {
    if (!originalBuffer) return;

    const excitedBuffer = applyExciter(originalBuffer);
    const wavBlob = bufferToWav(excitedBuffer);

    const url = URL.createObjectURL(wavBlob);
    const a = document.createElement('a');
    a.href = url;
    a.download = `${fileName}-excited.wav`;
    a.click();
