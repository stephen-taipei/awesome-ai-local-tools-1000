/**
 * Audio Vibrato - Tool #263
 * Add vibrato effect to audio (pitch modulation)
 */
...
const texts = {
    ...
    en: {
        title: 'Audio Vibrato',
        subtitle: 'Add vibrato effect to audio',
...
function init() {
    ...
    document.getElementById('downloadBtn').addEventListener('click', downloadVibrato);
}
...
function applyVibrato(buffer) {
    ...
    const maxDelay = depthSamples + 10;
    ...
    for (let ch = 0; ch < buffer.numberOfChannels; ch++) {
        ...
        for (let i = 0; i < buffer.length; i++) {
            ...
            const currentDelay = lfo * depthSamples;

            // Write current sample to delay buffer
            delayBuffer[writeIndex] = input[i];

            // Calculate read position with interpolation
            const readPos = writeIndex - currentDelay;
            const readPosWrapped = ((readPos % maxDelay) + maxDelay) % maxDelay;

            // Linear interpolation for smoother output
            const readIndex1 = Math.floor(readPosWrapped);
            const readIndex2 = (readIndex1 + 1) % maxDelay;
            const frac = readPosWrapped - readIndex1;

            const sample1 = delayBuffer[readIndex1];
            const sample2 = delayBuffer[readIndex2];
            const wet = sample1 + (sample2 - sample1) * frac;
...
async function startPreview() {
    ...
    const vibratoBuffer = applyVibrato(originalBuffer);

    sourceNode = audioContext.createBufferSource();
    sourceNode.buffer = vibratoBuffer;
...
async function downloadVibrato() {
    if (!originalBuffer) return;

    const vibratoBuffer = applyVibrato(originalBuffer);
    const wavBlob = bufferToWav(vibratoBuffer);

    const url = URL.createObjectURL(wavBlob);
    const a = document.createElement('a');
    a.href = url;
    a.download = `${fileName}-vibrato.wav`;
    a.click();
