/**
 * Audio Echo - Tool #313
 * Add multi-tap echo effect to audio
 */
...
const texts = {
    ...
    en: {
        title: 'Echo',
        subtitle: 'Add echo effect to audio',
...
function setupSliders() {
    document.getElementById('echoCount').addEventListener('input', (e) => { document.getElementById('countValue').textContent = e.target.value; });
    document.getElementById('interval').addEventListener('input', (e) => { document.getElementById('intervalValue').textContent = e.target.value + ' ms'; });
...
function switchLang(lang) {
    ...
    labels[0].textContent = t.echoCount;
    labels[1].textContent = t.interval;
...
async function processAudio() {
    ...
    const echoCount = parseInt(document.getElementById('echoCount').value);
    ...
    const extraLength = intervalSamples * echoCount;
    const outputLength = originalBuffer.length + extraLength;
    ...
    for (let ch = 0; ch < channels; ch++) {
        ...
        for (let e = 1; e <= echoCount; e++) {
            const echoGain = Math.pow(decay, e);
            const echoOffset = e * intervalSamples;

            for (let i = 0; i < inputData.length; i++) {
                const outIndex = i + echoOffset;
                if (outIndex < outputLength) {
                    outputData[outIndex] += inputData[i] * echoGain * mix;
                }
            }
        }
