...
function init() {
    ...
    document.getElementById('voicesSlider').addEventListener('input', (e) => {
    ...
    document.getElementById('downloadBtn').addEventListener('click', downloadChorus);
}
...
function switchLang(lang) {
    ...
    document.querySelectorAll('.option-group label')[3].textContent = t.voices;
    ...
    for (let ch = 0; ch < buffer.numberOfChannels; ch++) {
        ...
        for (let i = 0; i < buffer.length; i++) {
            ...
            for (let v = 0; v < voices; v++) {
                ...
                const lfo = Math.sin(2 * Math.PI * rate * t + phaseOffset);
                const currentDelay = delaySamples + depthSamples * lfo;

                const delayedIdx = Math.floor(i - currentDelay);
                if (delayedIdx >= 0 && delayedIdx < buffer.length) {
                    output[i] += input[delayedIdx] * (mix / voices);
                }
            }
        }
...
async function startPreview() {
    ...
    const chorusBuffer = applyChorus(originalBuffer);

    sourceNode = audioContext.createBufferSource();
    sourceNode.buffer = chorusBuffer;
...
async function downloadChorus() {
    if (!originalBuffer) return;

    const chorusBuffer = applyChorus(originalBuffer);
    const wavBlob = bufferToWav(chorusBuffer);

    const url = URL.createObjectURL(wavBlob);
    const a = document.createElement('a');
    a.href = url;
    a.download = `${fileName}-chorus.wav`;
    a.click();
