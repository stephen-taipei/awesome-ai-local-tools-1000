...
document.addEventListener('DOMContentLoaded', () => {
    ...
    losslessCheckbox.addEventListener('change', reconvertAll);
    ...
    function handleFiles(files) {
        ...
        convertedImages = [];
        originalFiles.forEach((file, index) => convertToWebP(file, index));
    }
    ...
    function convertToWebP(file, index) {
        ...
        reader.onload = (e) => {
            ...
            img.onload = () => {
                ...
                const quality = losslessCheckbox.checked ? 1 : parseInt(qualitySlider.value) / 100;
                const webpDataUrl = canvas.toDataURL('image/webp', quality);
                const originalName = file.name.replace(/\.[^/.]+$/, '');

                convertedImages[index] = { name: `${originalName}.webp`, dataUrl: webpDataUrl, originalSize: file.size };
                displayPreview(webpDataUrl, `${originalName}.webp`, file.size, index);
            };
    ...
    function displayPreview(dataUrl, name, originalSize, index) {
        const newSize = Math.round(dataUrl.split(',')[1].length * 0.75);
        ...
        div.innerHTML = `
            ...
            <div class="file-info">
                ...
                <div>WebP: ${formatSize(newSize)}</div>
    ...
    function downloadAll() {
        convertedImages.forEach((img, index) => {
            if (img) setTimeout(() => window.downloadSingle(index), index * 200);
        });
    }
